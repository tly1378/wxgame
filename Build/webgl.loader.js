...
Module.fetchWithProgress = function () {
  ...
  function loadBuild() {
    downloadFramework().then(function (tuanjieFramework) {
      ...
      RuntimeAPI = { Module: Module };
    }).then(() => {
    });

    var dataPromise = downloadBinary("dataUrl");
    Module.preRun.push(function () {
      RuntimeAPI.Module.addRunDependency("dataUrl");
      dataPromise.then(function (data) {
        ...
        while (pos < headerSize) {
          ...
          for (var folder = 0, folderNext = path.indexOf("/", folder) + 1 ; folderNext > 0; folder = folderNext, folderNext = path.indexOf("/", folder) + 1)
            RuntimeAPI.Module.FS_createPath(path.substring(0, folder), path.substring(folder, folderNext - 1), true, true);
          // for external boot.config
          if(path === "boot.config"){
            var request = new XMLHttpRequest();
            request.open('GET', "./boot.config", false); // Set the third parameter to false for synchronous requests
            request.send();
            if (request.status === 200) {
              var encoder = new TextEncoder();
              var convert = encoder.encode(request.responseText);
              RuntimeAPI.Module.FS_createDataFile(path, null, convert, true, true, true);
              console.log("find boot.config, custom boot");
              continue;
            } else {
              console.log("does not find boot.config, standard boot");
            }
          }
          RuntimeAPI.Module.FS_createDataFile(path, null, data.subarray(offset, offset + size), true, true, true);
        }
        RuntimeAPI.Module.removeRunDependency("dataUrl");
      });
    });
  }
